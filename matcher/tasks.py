...
def match_exercise(exercise_id, delay=True):
    ...
    for submission in exercise.non_staff_submissions:
...
def match_all_new_submissions_to_exercise(exercise_id, delay=True):
    ...
    compare_list = [s.as_dict() for s in exercise.non_staff_submissions]
...
def handle_match_results(matches):
    ...
    expected_result_count = exercise.non_staff_submissions.count()
