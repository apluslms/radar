...
def _match_all(config: dict[str, any], pairs_to_compare: tuple[dict[str, any], dict[str, any]]) -> list[any] | None:
    """
...
def match_all_combinations(config: dict[str, any], string_data_iter: list[dict[str, any]]) -> list[list[any]]:
    ...
    with Pool() as pool:
        results = pool.map(partial(_match_all, config), itertools.combinations(string_data_iter, 2))
...
def match_to_others(
    ...
    # Create a pool of workers to do the comparisons in parallel
    with Pool() as pool:
        results = pool.map(partial(_match_all, config), ((string_data, other_data) for other_data in other_data_iter))
