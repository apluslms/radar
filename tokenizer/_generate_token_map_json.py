...
def get_node_tokenizer_tokens(tokenizer_path):
    ...
    """
    node_print_tokentypes = "console.log(JSON.stringify(Array.from(require('{:s}').tokenTypes)))".format(tokenizer_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("js_tokenizer", type=str, help="Path to a Node.js JavaScript tokenizer")
    parser.add_argument("css_tokenizer", type=str, help="Path to a Node.js CSS tokenizer")
    args = parser.parse_args()
    all_tokens = (HTML_TOKEN_TYPES
                  + get_node_tokenizer_tokens(args.js_tokenizer)
                  + get_node_tokenizer_tokens(args.css_tokenizer))
    assert len(all_tokens) == len(set(all_tokens)), "The set of all tokens had less elements than the list of all tokens, are all tokens unique?"
